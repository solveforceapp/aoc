...
const PneumaCycle: React.FC<PneumaCycleProps> = ({ onOpenUniversalDirectory, onOpenCommaCorollary, onOpenDirectoryManager }) => {
    return (
        <div className="w-full h-full p-4 bg-black bg-opacity-30 backdrop-blur-sm rounded-lg border border-gray-700 pointer-events-auto flex flex-col justify-center items-center text-center">
            ...
            <div className="text-sm text-gray-400 space-y-2 bg-black/20 p-4 rounded-md border border-gray-800">
                <p>Access foundational system utilities for knowledge management and generation.</p>
            </div>
            <div className="mt-6 w-full space-y-3">
                ...
                >
                    <span className="bracket">[</span>UNIVERSAL DIRECTORY<span className="bracket">]</span>
                </button>
                 <button
                ...
                <button
                    onClick={onOpenDirectoryManager}
                    className="w-full px-4 py-3 text-sm font-bold transition-all duration-300 border-2 rounded-md font-orbitron bg-transparent border-emerald-800 hover:bg-emerald-700/50 hover:border-emerald-400 hover:text-white text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.3)] hover:shadow-[0_0_20px_rgba(16,185,129,0.6)]"
                >
                    <span className="bracket">[</span>MANAGE DIRECTORIES & TERMS<span className="bracket">]</span>
                </button>
            </div>
        </div>
    );
